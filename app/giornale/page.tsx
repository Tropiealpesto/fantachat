...
export default function GiornalePage() {
  ...
  const [title, setTitle] = useState<string>("Il Giornale FantaChat");
  const [content, setContent] = useState<string>("Nessun articolo ancora.");
  ...
  useEffect(() => {
    async function run() {
      ...
      // matchday selezionata: querystring oppure ultima
  ...
  useEffect(() => {
    async function loadArticle() {
      ...
      setTitle(art?.title ?? "Il Giornale FantaChat");
      setContent(art?.content ?? "Nessun articolo ancora per questa giornata.");
    }
    ...
    <>
      ...
      <main className="container">
        <div className="card" style={{ padding: 16, marginTop: 12 }}>
          ...
          {err && <div style={{ marginTop: 10, color: "var(--accent-dark)", fontWeight: 900 }}>{err}</div>}
