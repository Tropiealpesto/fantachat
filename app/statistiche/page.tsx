"use client";

import { useEffect, useMemo, useState } from "react";
import { useRouter } from "next/navigation";
import AppBar from "../components/AppBar";
import BottomNav from "../components/BottomNav";
import LoadingScreen from "../components/LoadingScreen";
import CompetitionBadge from "../components/CompetitionBadge";
import { useRequireApp } from "../hooks/useRequireApp";
import { rpcJson, fmt } from "../../lib/rpc";

type Row = { player_id: string; player_name: string; team_name: string | null; role: string; played_count: number; avg_points: number; total_points: number; best_points: number; worst_points: number };

const ROLE_COLORS: Record<string, { bg: string; color: string }> = {
  P: { bg: "#fef9c3", color: "#a16207" }, D: { bg: "#dcfce7", color: "#15803d" },
  C: { bg: "#dbeafe", color: "#1d4ed8" }, A: { bg: "#fee2e2", color: "#dc2626" },
};
const MEDALS: Record<number, string> = { 1: "🥇", 2: "🥈", 3: "🥉" };
function pLabel(r: Row) { return r.role === "P" ? (r.team_name || r.player_name) : r.player_name; }
function pSub(r: Row) { return r.role === "P" ? "Portiere" : (r.team_name ?? ""); }

export default function Statistiche() {
  const app = useRequireApp(true);
  const router = useRouter();
  const [rows, setRows] = useState<Row[]>([]);
  const [q, setQ] = useState("");
  const [showAll, setShowAll] = useState(false);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    if (!app.ready || !app.activeLeagueCompetitionId) return;
    rpcJson<Row[]>("get_player_stats", { p_league_competition_id: app.activeLeagueCompetitionId }, [])
      .then((r) => setRows(r ?? []))
      .finally(() => setLoading(false));
  }, [app.ready, app.activeLeagueCompetitionId]);

  const ranked = useMemo(() => rows.map((r, i) => ({ ...r, rank: i + 1 })), [rows]);
  const filtered = useMemo(() => {
    const needle = q.trim().toLowerCase();
    if (!needle) return ranked;
    return ranked.filter((r) => (r.player_name + " " + (r.team_name ?? "")).toLowerCase().includes(needle));
  }, [ranked, q]);
  const visible = showAll ? filtered : filtered.slice(0, 20);
  const maxAvg = useMemo(() => Math.max(1, ...rows.map((r) => Number(r.avg_points) || 0)), [rows]);

  if (!app.ready || loading) return <LoadingScreen />;
  const accent = app.competitionTheme.primary;

  return (
    <>
      <AppBar league={app.leagueName} team={app.teamName} onMenuOpen={app.openDrawer} />
      <main style={s.container}>
        <div style={s.head}>
          <CompetitionBadge name={app.competitionName} type={app.competitionType} />
          <h1 style={s.h1}>Statistiche</h1>
          <p style={s.hsub}>Media punti per giocatore in questa competizione</p>
          <input value={q} onChange={(e) => setQ(e.target.value)} placeholder="Cerca giocatore…" style={s.search} />
        </div>

        <div style={s.list}>
          {visible.map((r) => {
            const c = ROLE_COLORS[r.role] ?? ROLE_COLORS.C;
            const avg = Number(r.avg_points) || 0;
            const w = Math.max(0, Math.min(100, (avg / maxAvg) * 100));
            return (
              <button key={r.player_id} style={s.srow} onClick={() => router.push(`/giocatore/${r.player_id}`)}>
                <div style={s.top}>
                  <div style={s.rank}>{MEDALS[r.rank] ?? r.rank}</div>
                  <span style={{ ...s.rb, background: c.bg, color: c.color }}>{r.role}</span>
                  <div style={{ minWidth: 0 }}>
                    <div style={s.pn}>{pLabel(r)}</div>
                    <div style={s.pt}>{pSub(r)}</div>
                  </div>
                  <div style={{ textAlign: "right" }}>
                    <div style={{ ...s.avg, color: avg >= 0 ? accent : "#dc2626" }}>{fmt(avg)}</div>
                    <div style={s.avgl}>{r.played_count} {r.played_count === 1 ? "partita" : "partite"} a voto</div>
                  </div>
                </div>
                <div style={s.bar}><span style={{ ...s.barFill, width: `${w}%` }} /></div>
              </button>
            );
          })}
          {filtered.length === 0 && <div style={s.empty}>Nessun giocatore trovato.</div>}
        </div>

        {!showAll && filtered.length > 20 && (
          <button style={s.showall} onClick={() => setShowAll(true)}>Mostra tutti i giocatori ({filtered.length}) ›</button>
        )}
      </main>
      <BottomNav />
    </>
  );
}

const s: Record<string, React.CSSProperties> = {
  container: { maxWidth: 520, margin: "0 auto", padding: "16px 14px 100px", display: "grid", gap: 10 },
  head: { background: "white", border: "1px solid #e5e7eb", borderRadius: 18, padding: 16, boxShadow: "0 4px 16px rgba(15,23,42,.06)" },
  h1: { fontSize: 21, fontWeight: 1000, color: "#0f172a", margin: "10px 0 1px" },
  hsub: { fontSize: 12.5, color: "#64748b", fontWeight: 700, margin: 0 },
  search: { width: "100%", marginTop: 12, padding: "11px 13px", border: "1px solid #e5e7eb", borderRadius: 11, fontSize: 13, fontWeight: 700, fontFamily: "inherit", outline: "none" },
  list: { display: "grid", gap: 8 },
  srow: { background: "white", border: "1px solid #e5e7eb", borderRadius: 13, padding: "9px 11px", boxShadow: "0 2px 8px rgba(15,23,42,.04)", textAlign: "left", fontFamily: "inherit", cursor: "pointer", overflow: "hidden" },
  top: { display: "grid", gridTemplateColumns: "26px 26px 1fr auto", gap: 9, alignItems: "center" },
  rank: { fontSize: 14, fontWeight: 1000, color: "#64748b", textAlign: "center" },
  rb: { width: 26, height: 26, borderRadius: 8, display: "grid", placeItems: "center", fontWeight: 1000, fontSize: 11 },
  pn: { fontSize: 13.5, fontWeight: 1000, color: "#0f172a", overflow: "hidden", whiteSpace: "nowrap", textOverflow: "ellipsis" },
  pt: { fontSize: 10.5, fontWeight: 800, color: "#64748b" },
  avg: { fontSize: 17, fontWeight: 1000 },
  avgl: { fontSize: 9.5, fontWeight: 900, color: "#94a3b8" },
  bar: { height: 4, borderRadius: 3, background: "#eef2f5", marginTop: 8, marginLeft: 61, overflow: "hidden" },
  barFill: { display: "block", height: "100%", borderRadius: 3, background: "linear-gradient(90deg,#22c55e,#15803d)" },
  showall: { width: "100%", background: "white", border: "1px solid #e5e7eb", borderRadius: 12, padding: 11, fontWeight: 1000, color: "#15803d", fontSize: 13, cursor: "pointer", fontFamily: "inherit" },
  empty: { background: "white", border: "1px solid #e5e7eb", borderRadius: 13, padding: 16, color: "#64748b", fontWeight: 800 },
};